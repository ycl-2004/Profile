...
check(index.includes('base.css?v=orbit-popovers') && index.includes('layout.css?v=orbit-popovers') && index.includes('top-bar.js?v=orbit-popovers'), 'Orbit popover assets must be cache-busted together so new markup never loads with stale styles.');
...
check(index.includes('profile-actions.js?v=resume-pdf'), 'Résumé action code must be cache-busted when its destination changes.');
...
check(layoutStyles.includes('min-height: 32px') && layoutStyles.includes('width: 18px') && layoutStyles.includes('padding: 5px 8px'), 'Settings quick actions must use the compact density pass.');
